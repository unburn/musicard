...
async function Mini({
    ...
    progressBarColor,
    paused
}: MiniOptions): Promise<Buffer> {
    if (!backgroundColor) backgroundColor = "#000000";
    if (!menuColor) menuColor = "#e92725";
    if (!progress) progress = 10;
    if (!progressColor) progressColor = "#e92725";
    if (!progressBarColor) progressBarColor = "#0F0F0F";
    if (!thumbnailImage) thumbnailImage = `https://singlecolorimage.com/get/${menuColor.replace("#", "")}/544x544.png`;
    if (progress < 10) {
        progress = 10
